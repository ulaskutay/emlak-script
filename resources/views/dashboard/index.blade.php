            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 max-h-64 overflow-y-auto">
                ...
                <div class="flex items-center space-x-3 p-3 border border-gray-200 rounded-lg hover:bg-gray-50">
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-medium text-gray-900 truncate">{{ $agent->user->name }}</p>
                        <p class="text-xs text-gray-500">{{ $agent->active_listings_count }} Aktif İlan</p>
                    </div>
                    <a href="{{ route('agents.show', $agent) }}" class="px-3 py-1.5 text-xs bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex-shrink-0">
                        Görüntüle
                    </a>
                </div>
                @endforeach
